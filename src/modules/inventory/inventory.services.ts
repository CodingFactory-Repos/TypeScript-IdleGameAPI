...
> {
    ...
    let item;
    if (from === "marketplace") {
        item = await Marketplaces.findOne<Marketplace>({_id: new ObjectId(id)});
        if (!item) {
            return {message: "Item not found"};
        }

        item = await Shops.findOne<WithId<Shop>>({_id: new ObjectId(item.itemShopId)});
    } else {
        item = await Shops.findOne<WithId<Shop>>({
            _id: new ObjectId(id),
        });
    }
